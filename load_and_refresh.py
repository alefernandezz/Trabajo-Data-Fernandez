"""
ETL simple para el proyecto ventas-analytics.
...
Flujo:
  ...
  4. Es idempotente: se puede correr muchas veces sin duplicar datos
     (usa "upsert" manual sobre clientes/productos, y recrea pedidos/detalle
     en cada corrida a partir de la data cruda).
...
def normalizar(conn):
    ...
    productos_map = {}  # nombre -> producto_id
    ...
    # --- pedidos (un pedido = un order_id, con su primer email/fecha) ---
    cur.execute("""
    ...
    # --- detalle_pedido (una fila cruda = una línea de detalle) ---
    cur.execute("""
...
def crear_indices(conn):
    """Índices usados en 04_optimizacion.sql para acelerar los joins/agregaciones."""
    conn.executescript("""
...
def main():
    if DB_PATH.exists():
        DB_PATH.unlink()  # corrida limpia y reproducible
